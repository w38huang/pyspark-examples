...
def run():
    ...
    model = LinearRegression().setFeaturesCol(featuresColumn).setLabelCol(labelColumn).setPredictionCol(predictionColumn)
    ...
    params = ParamGridBuilder().addGrid(model.elasticNetParam, [0.2, 0.5, 0.8]).addGrid(model.regParam, [0.1, 0.5, 0.9]).build()
