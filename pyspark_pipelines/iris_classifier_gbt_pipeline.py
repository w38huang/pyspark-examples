...
# note this only spuport binary classification, not multiple classification
# TODO:
def run():
    ...
    model = GBTClassifier().setFeaturesCol(featuresColumn).setLabelCol(labelColumn).setPredictionCol(predictionColumn)
    ...
    params = ParamGridBuilder().addGrid(model.maxDepth, [1, 2, 3, 4, 5]).build()
